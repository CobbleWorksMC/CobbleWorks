...
public class AncientStaff extends Item {
	...
	public void onUpdate(ItemStack stack, World worldIn, Entity caster, int itemSlot, boolean isSelected) {
		...
		if (!stack.hasTagCompound()) {
			stack.setTagCompound(new NBTTagCompound());
			stack.getTagCompound().setInteger("cd", 0);
			stack.getTagCompound().setInteger("delay", 0);
			stack.getTagCompound().setDouble("X", 0);
			stack.getTagCompound().setDouble("Y", 0);
			stack.getTagCompound().setDouble("Z", 0);
		}
		
		int cd = stack.getTagCompound().getInteger("cd");
		this.delay = stack.getTagCompound().getInteger("delay");
		...
		stack.getTagCompound().setInteger("delay", this.delay);
	}
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack is, World worldIn, EntityPlayer caster, EnumHand hand) {
		...
		if (is.getTagCompound().getInteger("cd") == 0) {
			boolean bool = iceBarrage(caster, is);
			if (bool) {
				...
				is.getTagCompound().setInteger("delay", this.maxDelay);
	...
	public boolean iceBarrage(EntityPlayer caster, ItemStack staff) {
		...
		this.pos = primeTarget.getPosition();
		
		staff.getTagCompound().setDouble("X", pos.getX());
		staff.getTagCompound().setDouble("Y", pos.getY());
		staff.getTagCompound().setDouble("Z", pos.getZ());
		/*
    	this.targets = (caster.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(pos.getX()-2, pos.getY()-2, pos.getZ()-2, pos.getX()+2, pos.getY()+2, pos.getZ()+2)));
    	...
    	*/
    	//TO FIX: use itemstack nbt to store the list of targets instead of the item itself
	...
	public boolean actuallySpellcast(Entity caster) {
		...
		for (EntityLivingBase target : targets) {
    		if (target != caster) {
    			...
    			eib = new EntityIceBarrage(caster.worldObj, target);
