...
public class TileEntityPendulumSummoner extends TileEntity implements ITickable, IInventory {
	...
	public void update() {
		...
		int power = worldObj.isBlockIndirectlyGettingPowered(pos);
		if (power > 7) {
			return;
		}
			...
			if (contents[i] != null) {
				if (contents[i].getItem() instanceof ItemMobCard) {
					...
					if (isBetween(blueScale.getScale(contents[0]), summon.getLevel(contents[i]), redScale.getScale(contents[6]))) {
						...
						IEntityLivingData data = new IEntityLivingData() {};
						
						if (elb instanceof EntitySkeleton) {
							if (elb.dimension == -1) {
								((EntitySkeleton) elb).setSkeletonType(SkeletonType.WITHER);;
							}
						}
						
						worldObj.spawnEntityInWorld(elb);
						((EntityLiving) elb).onInitialSpawn(worldObj.getDifficultyForLocation(pos), data);
					}
				}
			}
		}
