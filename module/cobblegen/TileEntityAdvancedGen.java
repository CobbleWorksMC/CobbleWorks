...
public class TileEntityAdvancedGen extends TileEntityCobbleGen implements IFluidHandler {
	...
	private static final float[] lavaConsumed = new float[] {0f, 1f, .5f, .1f, .1f, 1.0f, .1f, .1f, 10f};
	...
	public ItemStack generateHelper(int index, int amount) {
		...
		case 7: return new ItemStack(Blocks.PRISMARINE, 		amount);
	...
	public void update() {
		...
		if (power.getEnergyStored() >= EnergyPerTick && ticksSinceEvent < 100 && !allZero() && calcLavaCost()) {
			ticksSinceEvent++;
