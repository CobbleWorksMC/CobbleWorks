...
public class BlockCustomgen extends BlockContainer {

	public BlockCustomgen() {
		super(Material.ANVIL);
		this.setUnlocalizedName(Cobbleworks.MODID + ".customgen");
		this.setRegistryName("customgen");
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new TileEntityCustomgen();
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if(worldIn.isRemote){
			...
			if (te instanceof TileEntityCustomgen) {
				if (((TileEntityCustomgen)te).isEnabled) {
					if (playerIn.getHeldItem(hand).getItem() == Items.LAVA_BUCKET && (((TileEntityCustomgen)te).fill(new FluidStack(FluidRegistry.LAVA, 1000), false) == 1000)) {
						((TileEntityCustomgen)te).fill(new FluidStack(FluidRegistry.LAVA, 1000), true);
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {

        {
            ...
            if (tileentity instanceof TileEntityCustomgen) {
