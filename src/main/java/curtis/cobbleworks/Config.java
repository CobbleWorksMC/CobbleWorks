	...
	public static void load() {
		...
		try {
			...
			if (enableCustomgen) {
				...
				customTiers = cfg2.get("general", "customTiers", customTiers, "Use this to change the tiers the Custom Cobbleworks produces items at. \nThere needs to be nine values.").getIntList();
				customPower = cfg2.get("general", "customPower", customPower, "Set the power consumed at each tier by the Custom Cobbleworks. \nThere needs to be six values.").getIntList();
				customLava = cfg2.get("general", "customLava", customLava, "Use this to set the lava consumed per tick, per item or block created, in micro-buckets (a value of 1000 will be 1mb/t per item). \nThere needs to be nine values").getIntList();
			}
